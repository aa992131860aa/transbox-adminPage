...
gulp.task('injectAuth', ['stylesAuth'], function () {
  return injectAlone({
    ...
    paths: [path.join(conf.paths.src, '/auth.html'), path.join(conf.paths.src, '/reg.html'), path.join(conf.paths.src, '/pwd.html'),path.join(conf.paths.src, '/shulanyiyuan.html'),path.join(conf.paths.src, '/zhejiangdaxuediyifushuyiyuan.html')]
  })
});
